...
if ( $attributes['enableOptions']['heading'] || $attributes['enableOptions']['subHeading'] ) {
	...
	if ( $attributes['enableOptions']['subHeading'] ) {
		$subheading_tag   = isset( $attributes['subHeading']['tag'] ) && in_array( $attributes['subHeading']['tag'], $valid_tags, true ) ? sanitize_text_field( $attributes['subHeading']['tag'] ) : 'p';
		$subheading_label = isset( $attributes['subHeading']['label'] ) ? $attributes['subHeading']['label'] : '';
		$output          .= sprintf(
			'<%1$s class="cozy-block-magazine-grid__sub-heading">%2$s</%1$s>',
