...
wp_add_inline_script( 'cozy-block-scripts', 'document.addEventListener("DOMContentLoaded", function(event) { window.cozyBlockFeaturedPost( "' . esc_html( $client_id ) . '" ) }) ' );
...
if ( ! function_exists( 'cozy_block_featured_post_enqueue_google_fonts' ) ) {
	function cozy_block_featured_post_enqueue_google_fonts( $attributes ) {
		$font_families = array();
		...
		if ( ! empty( $font_query ) ) {
			...
			wp_add_inline_style( 'cozy-block--featured-post--style', '@import url("' . rawurldecode( esc_url( $google_fonts_url ) ) . '");' );
		}
	}
}
...
add_action(
	...
	function () use ( $block_styles, $attributes ) {
		cozy_block_featured_post_enqueue_google_fonts( $attributes );

		wp_add_inline_style( 'cozy-block--featured-post--style', esc_html( $block_styles ) );
	}
);
