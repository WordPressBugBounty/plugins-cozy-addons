...
wp_add_inline_script( 'cozy-block-scripts', 'document.addEventListener("DOMContentLoaded", function(event) { window.cozyBlockTrendingPostsInit( "' . esc_html( $client_id ) . '" ) }) ' );
...
if ( ! function_exists( 'cozy_block_trending_post_enqueue_google_fonts' ) ) {
	function cozy_block_trending_post_enqueue_google_fonts( $attributes ) {
		$font_families = array();
...
add_action(
	...
	function () use ( $block_styles, $attributes ) {
		cozy_block_trending_post_enqueue_google_fonts( $attributes );
