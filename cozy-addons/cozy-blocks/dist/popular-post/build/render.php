...
if ( ! function_exists( 'cozy_block_popular_post_enqueue_google_fonts' ) ) {
	function cozy_block_popular_post_enqueue_google_fonts( $attributes ) {
		$font_families = array();
		...
		if ( ! empty( $font_query ) ) {
			...
			wp_add_inline_style( 'cozy-block--popular-post--style', '@import url("' . rawurldecode( esc_url( $google_fonts_url ) ) . '");' );
		}
	}
}
...
add_action(
	...
	function () use ( $block_styles, $attributes ) {
		cozy_block_popular_post_enqueue_google_fonts( $attributes );

		wp_add_inline_style( 'cozy-block--popular-post--style', esc_html( $block_styles ) );
	}
);
